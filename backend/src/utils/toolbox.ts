...
const Tools = {
    ...
    /**
     * @Description - Check if a today is x days from a given date
     * @param {Date} date - The date to check against
     * @param {number} days - The number of days to check against
     * @returns {boolean} - True if today is x days from the given date, false otherwise
     */
    isXDaysFromNow(date: Date, days: number): boolean {
        const today = moment();
        const dateToCheck = moment(date);
        const difference = today.diff(dateToCheck, 'days');
        return difference === days;
    },

    /**
     * @description - convert a given amount to decimal
     * @param {string} amount - the amount to convert
     * @returns
     */
    toDecimal(amount: string): number {
        return Number(!amount ? 0.0 : Number(amount?.replace(/,/g, ''))?.toFixed(2));
    },
};
